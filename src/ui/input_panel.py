...
class InputPanel(QWidget):
    ...
    def __init__(self):
        ...
        # Nút Tùy chọn (MainWindow sẽ gán Style và Menu sau)
        ...
        checkbox_style = """
            QCheckBox { font-size: 16px; spacing: 10px; color: #2C3E50; min-height: 20px; }
            QCheckBox::indicator:unchecked { border: 2px solid #BDC3C7; border-radius: 4px; background-color: white; width: 20px; height: 20px; }
            QCheckBox::indicator:checked { border: none; background-color: transparent; image: url(assets/icons/tick_xanh.png); width: 24px; height: 24px; }
        """

        f_lay.addWidget(QLabel("<b>🍂 BƯỚC 2: CHỌN MÙA <font color='red'>*</font></b>"))
        self.group_mua = QButtonGroup(self); self.group_mua.setExclusive(True)
        grid_mua = QGridLayout(); mua_opts = ["Mùa nước nổi", "Mùa mưa", "Mùa khô", "Mùa Tết", "Mùa trái cây"]
        for i, text in enumerate(mua_opts):
            chk = QCheckBox(text); chk.setStyleSheet(checkbox_style); self.group_mua.addButton(chk); grid_mua.addWidget(chk, i // 2, i % 2)
        f_lay.addLayout(grid_mua)

        f_lay.addWidget(QLabel("<b>🍲 BƯỚC 3: LOẠI MÓN ĂN <font color='red'>*</font></b>"))
        self.group_loai = QButtonGroup(self); self.group_loai.setExclusive(True)
        grid_loai = QGridLayout(); loai_opts = ["Món nước", "Món khô", "Món tráng miệng", "Món gỏi", "Món nướng", "Món xào", "Món hấp", "Món lẩu", "Món cháo", "Món bánh"]
        for i, text in enumerate(loai_opts):
            chk = QCheckBox(text); chk.setStyleSheet(checkbox_style); self.group_loai.addButton(chk); grid_loai.addWidget(chk, i // 2, i % 2)
        f_lay.addLayout(grid_loai)
        
        f_lay.addWidget(QLabel("<b>🐟 BƯỚC 4: NGUYÊN LIỆU CHÍNH <font color='red'>*</font></b>"))
        self.combo_nlc = QComboBox(); self.combo_nlc.addItems(["Tất cả", "Hải sản", "Cá", "Bún", "Hủ tiếu", "Bánh tằm", "Bột", "Nếp", "Gạo", "Thịt", "Trứng", "Trái cây"])
        f_lay.addWidget(self.combo_nlc)

        f_lay.addWidget(QLabel("<b>🌿 BƯỚC 5: NGUYÊN LIỆU PHỤ <font color='red'>*</font></b>"))
        self.combo_nlp = QComboBox(); self.combo_nlp.addItems(["Tất cả", "Sen", "Mắm", "Bông điên điển", "Lá chúc", "Rau đắng", "Nước cốt dừa", "Rau củ quả", "Chao", " Lá giang"])
        f_lay.addWidget(self.combo_nlp)

        f_lay.addWidget(QLabel("<b>👅 Bước 6: Chọn khẩu vị <font color='red'>*</font></b>"))
        self.group_vi = QButtonGroup(self); self.group_vi.setExclusive(True); grid_vi = QGridLayout()
        vi_options = [("🌶️ Cay", "Cay"), ("🍰 Ngọt", "Ngọt"), ("🍋 Chua", "Chua"), ("🧂 Mặn", "Mặn"), ("🥥 Béo", "Béo"), ("🍃 Thanh", "Thanh"), ("🥜 Bùi", "Bùi"), ("☕ Đắng", "Đắng")]
        for i, (display, value) in enumerate(vi_options):
            chk = QCheckBox(display); chk.setStyleSheet(checkbox_style); self.group_vi.addButton(chk); grid_vi.addWidget(chk, i // 2, i % 2)
        ...
        bottom_buttons_layout.setContentsMargins(0, 15, 0, 10)

        self.btn_back = QPushButton("⬅ QUAY LẠI TRANG CHỦ")
        self.btn_back.setCursor(Qt.PointingHandCursor)
        self.btn_back.setStyleSheet("QPushButton { border: none; color: #1976D2; font-weight: bold; font-size: 15px; text-decoration: underline; padding: 10px; } QPushButton:hover { color: #1565C0; }")

        self.btn_submit = QPushButton("XEM GỢI Ý MÓN ĂN 🍽️")
        self.btn_submit.setFixedSize(220, 50)
        self.btn_submit.setCursor(Qt.PointingHandCursor)
        self.btn_submit.setStyleSheet("QPushButton { background-color: #2E7D32; color: white; font-weight: bold; font-size: 16px; border-radius: 8px; } QPushButton:hover { background-color: #1B5E20; }")

        bottom_buttons_layout.addWidget(self.btn_back)
        bottom_buttons_layout.addStretch()
        bottom_buttons_layout.addWidget(self.btn_submit)
